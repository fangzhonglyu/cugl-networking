...
def reassign_pbxproj(config,pbxproj):
    ...
    wrtcdir = '../'+util.path_to_posix(os.path.join(config['build_to_cugl'],'thirdparty','libdatachannel'))
    ...
    for pos in range(len(section)):
        ...
        if '__WEBRTC_INCLUDE__' in section[pos]:
            section[pos] = section[pos].replace('__WEBRTC_INCLUDE__','"$(SRCROOT)/'+wrtcdir+'/include"')
